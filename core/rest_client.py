...
class RestClient():
    ...
    def request(self, url, method, data=None, json=None, **kwargs):
        ...
        if method == "POST":
            return requests.post(url, data, json, **kwargs , verify=False)
        if method == "PUT":
            if json:
                # PUT 和 PATCH 中没有提供直接使用json参数的方法，因此需要用data来传入
                data = complexjson.dumps(json)
            return self.session.put(url, data, **kwargs,verify=False)
        if method == "DELETE":
            return self.session.delete(url, **kwargs,verify=False)
        if method == "PATCH":
            if json:
                data = complexjson.dumps(json)
            return self.session.patch(url, data, **kwargs,verify=False)
